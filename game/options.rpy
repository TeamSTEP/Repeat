...
init -1 python hide:
    ...
    config.window_title = u"리피트 - 회전하는 추억"
    ...
    style.window.background = Frame("system/box_paper.png", 0, 40)

    ## margin은 대사창의 배경 그림이 그려지지 않는
    ## 대사창 주변의 여백입니다.

    # style.window.left_margin = 40
    # style.window.right_margin = 40
    ...
    # 기본 폰트: 나눔 고딕
    # 경로: 프로젝트폴더/tl/None/
    style.default.font = "fonts/RepeatFont.ttf"
    style._default.font = "fonts/RepeatFont.ttf"
    ...
    style.button.activate_sound = "se/menu_click.ogg"
    style.imagemap.activate_sound = "se/menu_click.ogg"

    ## 게임 메뉴에 진입하거나 빠져나올 때 재생하는 효과음.

    config.enter_sound = "se/menu_click.ogg"
    config.exit_sound = "se/menu_click.ogg"

    ## 효과음 볼륨을 체크할 때 재생되는 샘플 효과음.

    config.sample_sound = "se/logo_teamstep.mp3"

    ## 사용자가 메인 메뉴에 있을 때 재생되는 음악.

    config.main_menu_music = "bgm/smooth5.mp3"
    ...
    #########################################
    ## 화면 전환 효과
    # 전환 효과 목록: http://www.renpy.org/doc/html/transitions.html#Pixellate
...
init -1 python hide:
    ...
    ## 모바일에서 사용될 터치 제스쳐를 정의 합니다.
    #config.dispatch_gesture = None
    config.gestures = { "n_s_w_e_w_e" : "progress_screen", "s": "hide_windows", "w_e" : "game_menu" }
    ...
    config.autosave_frequency = 400
    
    ##대사 뒤로 넘기기 설정
    config.rollback_side_size = 0
    ...
    style.menu_choice_button.background = Frame("system/choice_bg_idle.png",28,9)
    style.menu_choice_button.hover_background = Frame("system/choice_bg_hover.png",28,9)
    ...
    style.menu_choice_button.yminimum = 117 #Sets height, recommended to be the exact height of your image
    
    ##Modifies the Menu Choice's Text
    style.menu_choice.color = "#ffffff"
    style.menu_choice.size = 30
    style.menu_choice.outlines = [(1, "#000000", 0, 0)]
    # style.menu_choice.hover_color = "#fff"
    # style.menu_choice.hover_outlines = [(2, "#000", 0, 0)]
    
    style.file_picker_button.color = "#575757"
    ...
    #All frames
    style.frame.background = Frame("system/frame.png",10,10)
    
    #All buttons
    style.button.background = Frame("system/button_idle.png",10,10)
    style.button.hover_background = Frame("system/button_hover.png",10,10)
    style.button.selected_background = Frame("system/button_selected.png",10,10)
    style.button.selected_hover_background = Frame("system/button_hover.png",10,10)
    style.button.yminimum = 40
    
    style.button_text.color = "#fff"
    style.button_text.size = 28#48
    ...
    style.default.hover_outlines = [(2, "#f2f2f2", 0, 0)]
    style.default.selected_outlines = [(1, "#978686", 0, 0)]
    style.default.selected_hover_outlines = [(2, "#fbfbfb", 0, 0)]
    

    ##This will change the save/load slots.
    #style.large_button.background = Frame("system/button_idle.png",10,10)
    #style.large_button.hover_background = Frame("system/button_hover.png",10,10)
    #style.large_button.selected_background = Frame("system/button_selected.png",10,10)
    #style.large_button.selected_hover_background = Frame("system/button_hover.png",10,10)
    #style.large_button.yminimum = 80
    #style.large_button.ypadding = 20
...
init python:
    ...
    build.directory_name = "Repeat-1.0"
    ...
    build.archive("trash", "all")

    # Put script files into the scripts archive.
    build.classify("game/**.rpyc", "data")
    build.classify("game/**.rpy", "trash")

    # Put images into the data archive.
    build.classify("game/**.jpg", "data")
    build.classify("game/**.png", "data")
    
    # Put sounds into the data archive.
...
#################백그라운드 사운드 효과 목록#############
    ...
    renpy.music.register_channel("channel1", "sfx", True)
##################안드로이드 설정##################
    build.google_play_key = ""
